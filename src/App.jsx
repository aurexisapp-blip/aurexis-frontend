...
// ── Native "brand new iPhone" design system (2026 redesign) ────────────
// One flat token set + list-row primitive shared by every native screen
// (Dashboard/Watchlist/Screener/Journal/Settings) so the whole app reads
// as one deliberate system. Scoped to native only via isNative checks at
// each call site -- desktop/web-mobile keep the existing T/App.css theme.
const NATIVE_COLOR = {
  bg: "#0a0a0a",
  surface: "#111110",
  divider: "#1a1a19",
...
const NATIVE_TYPE = {
  hero: { fontSize: 33, fontWeight: 600, letterSpacing: "-0.01em", color: NATIVE_COLOR.text },
  ...
  rowSubtitle: { fontSize: 12, fontWeight: 500, color: NATIVE_COLOR.textLabel },
  label: { fontSize: 11, fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase", color: NATIVE_COLOR.textFaint },
};

// Conviction tier -> score pill color. Green for cleared-threshold tiers,
// amber for moderate, neutral gray otherwise -- matches the "green/amber
// only, never a 3rd unspecified accent" rule in the design brief.
function nativeScorePillColors(tier) {
  ...
  if (t === "MODERATE") {
    return { bg: "rgba(233,180,89,0.16)", text: NATIVE_COLOR.warning };
  }
  return { bg: "rgba(245,245,244,0.08)", text: NATIVE_COLOR.textLabel };
}
...
// title+subtitle stack, trailing stat/pill/chevron, 0.5px divider below
// (owner passes `last` to suppress it on a list's final row).
function NativeListRow({ avatar, avatarColor, title, subtitle, trailing, trailingSub, trailingColor, chevron, onClick, last }) {
  return (
    <div
      onClick={onClick}
      style={{
    ...
    >
      {avatar !== undefined && (
        <div style={{
          ...
          background: avatarColor || "rgba(245,245,244,0.08)",
      ...
      {chevron && <span style={{ color: NATIVE_COLOR.textFaint, fontSize: 16, flexShrink: 0, marginLeft: trailing != null ? 2 : 0 }}>›</span>}
    </div>
  );
}

// ── SVG nav icons ────────────────────────────────────────────────────────
// Real stroke-based SVGs (not emoji) so `stroke`/`fill` can actually be
// recolored per active/inactive state via currentColor.
...
const NATIVE_TAB_ICONS = { dashboard: IconFlame, watchlist: IconStar, screener: IconFilter, tradejournal: IconNotebook, settings: IconGear };
...
async function loadWatchlistLive() {
  ...
  const Dashboard = () => {
    ...
    const AnalyzeHistoryCard = () => {
      ...
      if (isNative) {
        return (
          <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: `${NATIVE_SPACE.m}px ${NATIVE_SPACE.l}px` }}>
        ...
        >
          ...
          <div className="heroBody" style={isNative ? { padding: `${NATIVE_SPACE.xl}px ${NATIVE_SPACE.l}px ${NATIVE_SPACE.l}px` } : undefined}>
            ...
            {canAccess(userPlan, "starter") && !canAccess(userPlan, "pro") && isNative ? (
              /* Starter on native: one unified surface, blurred -- same
                 shape as the unlocked card below, not 5 separate boxes. */
              <div style={{ position: "relative", borderRadius: 20, overflow: "hidden", marginTop: NATIVE_SPACE.m, marginBottom: NATIVE_SPACE.m }}>
              ...
              /* Native, unlocked: ONE surface card, Entry/Stop/T1/T2/T3 as
                 hairline-divided rows -- this is the exact structure the
                 mockup calls out. Position/Horizon/Regime stat row below
                 is 3 small cards (not pills). Rationale/risk/trailing-stop
                 content moves to the Pick Detail screen (View full analysis)
                 so the dashboard stays minimal -- nothing is dropped. */
              <div style={{ marginTop: NATIVE_SPACE.m, marginBottom: NATIVE_SPACE.m }}>
                <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, overflow: "hidden" }}>
                  {[
                ...
                {/* ── 3-stat row: Position size / Time horizon / Market regime, small cards ── */}
                <div style={{ display: "flex", gap: NATIVE_SPACE.s, marginTop: NATIVE_SPACE.m }}>
                  {[
                    ...
                    <div key={s.label} style={{ flex: 1, background: NATIVE_COLOR.surface, borderRadius: 14, padding: `${NATIVE_SPACE.s}px ${NATIVE_SPACE.m}px`, textAlign: "center" }}>
                ...
                {(pickRationale.length > 0 || riskFlags.length > 0 || trailingStopPlan.length > 0) && (
                  <div
                    ...
                    style={{
                      ...
                      background: NATIVE_COLOR.surface, borderRadius: 16, cursor: "pointer",
                    }}
                  >
    ...
    // standalone TopMoversCard/Movers-tab data into one list -- both read
    // the same `movers` state and showed overlapping data as two separate
    // cards (plus a 6th nav tab). Nothing here is deleted, just relocated.
    ...
    if (isNative && pickDetailOpen) {
      // Recompute the same bestPickData-derived fields HeroCard uses for
      // rationale/risk/trailing-stop -- this content moved off Hero to keep
      // the dashboard minimal, but nothing was dropped, just relocated here.
        ...
        <div style={{ background: NATIVE_COLOR.bg, minHeight: "100%" }}>
          ...
          <div style={{ padding: `0 ${NATIVE_SPACE.l}px ${NATIVE_SPACE.xl}px`, display: "flex", flexDirection: "column", gap: NATIVE_SPACE.m }}>
              ...
              <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: `${NATIVE_SPACE.m}px ${NATIVE_SPACE.l}px` }}>
                {_pickRationale.length > 0 && (
                  <div style={{ marginBottom: (_riskFlags.length || _trailingStopPlan.length) ? NATIVE_SPACE.m : 0 }}>
                    <div style={NATIVE_TYPE.label}>Why this setup</div>
                    {_pickRationale.map((r, i) => (
                      <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 6 }}>
                        <span style={{ color: NATIVE_COLOR.accent, fontSize: 10, marginTop: 3, flexShrink: 0 }}>●</span>
                        <span style={{ fontSize: 13, color: NATIVE_COLOR.textLabel, lineHeight: 1.5 }}>{r}</span>
                      </div>
                    ))}
                  </div>
                )}
                {_trailingStopPlan.length > 0 && (
                  <div style={{ marginBottom: _riskFlags.length ? NATIVE_SPACE.m : 0, paddingTop: _pickRationale.length ? NATIVE_SPACE.m : 0, borderTop: _pickRationale.length ? `0.5px solid ${NATIVE_COLOR.divider}` : "none" }}>
                    <div style={NATIVE_TYPE.label}>Trailing stop plan</div>
                    {_trailingStopPlan.map((step, i) => (
                      <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 6 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: NATIVE_COLOR.warning, minWidth: 40, flexShrink: 0 }}>+{step.trigger_pct}%</span>
                        <span style={{ fontSize: 13, color: NATIVE_COLOR.textLabel, lineHeight: 1.5 }}>{step.action}</span>
                      </div>
                    ))}
                  </div>
                )}
                {_riskFlags.length > 0 && (
                  <div style={{ paddingTop: (_pickRationale.length || _trailingStopPlan.length) ? NATIVE_SPACE.m : 0, borderTop: (_pickRationale.length || _trailingStopPlan.length) ? `0.5px solid ${NATIVE_COLOR.divider}` : "none" }}>
                    <div style={{ ...NATIVE_TYPE.label, color: NATIVE_COLOR.negative }}>Watch out for</div>
                    {_riskFlags.map((f, i) => (
                      <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 6 }}>
                        <span style={{ color: NATIVE_COLOR.negative, fontSize: 10, marginTop: 3, flexShrink: 0 }}>▲</span>
                        <span style={{ fontSize: 13, color: NATIVE_COLOR.negative, lineHeight: 1.5 }}>{f}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {analyzeIsLowConviction ? (
              <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: NATIVE_SPACE.l }}>
                <div style={{ ...NATIVE_TYPE.rowTitle, color: NATIVE_COLOR.warning, marginBottom: 4 }}>⚠ Low Conviction — Trade Not Recommended</div>
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: NATIVE_SPACE.l, padding: NATIVE_SPACE.l }}>
          ...
          <div>
            ...
            <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: `0 ${NATIVE_SPACE.l}px` }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: NATIVE_SPACE.s }}>
              <div style={NATIVE_TYPE.label}>System candidates</div>
  ...
  const Screener = () => {
    ...
    if (isNative) {
      // No filter/category/tag data exists anywhere in this screen's model
      // (screenOne only returns aiScore/momentum/trend/confidence/direction
      // per symbol) -- so the chip row drives the existing sort-by-column
      // state (COL_HEADERS) instead of inventing categories that aren't
      // real. Active chip = current sort column, in accent green.
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: NATIVE_SPACE.l, padding: NATIVE_SPACE.l }}>
          ...
          <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: NATIVE_SPACE.l }}>
            <div style={{ display: "flex", alignItems: "center", background: "rgba(245,245,244,0.06)", borderRadius: 14, padding: "0 14px", gap: 8, marginBottom: NATIVE_SPACE.m }}>
            ...
            <motion.button
              ...
              onClick={runScreen}
              disabled={loading}
              style={{ width: "100%", padding: "12px 0", borderRadius: 14, background: NATIVE_COLOR.accentBase, border: "none", color: "#06120c", fontSize: 15, fontWeight: 700, cursor: "pointer" }}
            >
          ...
          {!loading && hasScreened && results.length > 0 && (
            <>
              {/* ── Sort-by chips (horizontally scrollable, active = green) ── */}
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: active ? "rgba(62,224,163,0.14)" : "rgba(245,245,244,0.06)",
                        color: active ? NATIVE_COLOR.accent : NATIVE_COLOR.textLabel,
    ...
    if (isNative) {
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: NATIVE_SPACE.l, padding: NATIVE_SPACE.l }}>
          ...
          {total > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", background: NATIVE_COLOR.surface, borderRadius: 20, padding: NATIVE_SPACE.l }}>
          ...
          <motion.button
            ...
            style={{ width: "100%", padding: "13px 0", borderRadius: 16, background: journalAddOpen ? "transparent" : NATIVE_COLOR.accentBase, border: journalAddOpen ? `0.5px solid ${NATIVE_COLOR.divider}` : "none", color: journalAddOpen ? NATIVE_COLOR.text : "#06120c", fontSize: 15, fontWeight: 700, cursor: "pointer" }}
          >
            {journalAddOpen ? "Cancel" : "+ Add Trade"}
          </motion.button>

          {journalAddOpen && (
            <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: NATIVE_SPACE.l, display: "flex", flexDirection: "column", gap: NATIVE_SPACE.s }}>
              <div style={{ position: "relative" }}>
                <input
                  ref={symRef}
                  style={{ ...inp, width: "100%", background: "rgba(245,245,244,0.06)", border: "none", paddingRight: fetchingSymbol ? 28 : undefined }}
                  placeholder="Symbol"
              ...
              {[
                ...
                <input
                  key={fkey} ref={ref}
                  style={{ ...inp, width: "100%", background: "rgba(245,245,244,0.06)", border: "none" }}
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={submitAdd} style={{ padding: "12px 0", borderRadius: 14, background: NATIVE_COLOR.accentBase, border: "none", color: "#06120c", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Submit</motion.button>
            </div>
          )}

          {/* Journal's real data model is only ever open/won/lost (see
              submitAdd/submitClose above) -- each gets a distinct pill
              color/label. The separate 7-status pick-outcome enum
              (pending/won/won_drift/lost/lost_drift/expired_neutral/expired)
              belongs to a different, currently-unrendered surface
              (recentPicksData) and isn't part of this screen's data. */}
          <div style={{ background: NATIVE_COLOR.surface, borderRadius: 20, padding: journalTrades.length ? `0 ${NATIVE_SPACE.l}px` : NATIVE_SPACE.l }}>
              ...
              return (
                <div key={trade.id}>
                  <NativeListRow
                    ...
                    trailing={<NativeScorePill label={statusLabel} tier={trade.status === "won" ? "HIGH" : trade.status === "lost" ? "LOW" : "MODERATE"} />}
                  ...
                  {isClosing && (
                    <div style={{ display: "flex", gap: NATIVE_SPACE.s, padding: `0 0 ${NATIVE_SPACE.m}px` }}>
                      <input
                        style={{ ...inp, flex: 1, background: "rgba(245,245,244,0.06)", border: "none" }}
                      ...
                      />
                      <button onClick={() => submitClose(trade.id)} style={{ padding: "0 16px", borderRadius: 12, background: NATIVE_COLOR.accentBase, border: "none", color: "#06120c", fontWeight: 700, cursor: "pointer" }}>✓</button>
                      <button onClick={() => deleteTrade(trade.id)} style={{ padding: "0 12px", borderRadius: 12, background: "transparent", border: `0.5px solid ${NATIVE_COLOR.divider}`, color: NATIVE_COLOR.textFaint, cursor: "pointer" }}>⌫</button>
                    </div>
                  )}
  ...
  const Settings = () => {
    ...
    const FieldRow = ({ label, value, action, chevron }) => (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 0", borderBottom: isNative ? `0.5px solid ${NATIVE_COLOR.divider}` : (dm ? "1px solid rgba(255,255,255,0.05)" : "1px solid rgba(0,0,0,0.06)"), gap: 10 }}>
        <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          {action}
          {isNative && chevron && <span style={{ color: NATIVE_COLOR.textFaint, fontSize: 16 }}>›</span>}
        </div>
      </div>
    );
    ...
    const FEATURE_CATEGORIES = [
      ...
      },
      {
        ...
        features: [
          ...
          { key: "email_alerts",   label: "Email alerts — new picks & outcomes" },
    ...
    <div className="appShell">
      <div className="layout" style={{ display: "flex" }}>
        ...
        {(() => {
          ...
          // Native: exactly 5 tabs. Movers is not a nav destination here --
          // its data lives inside Dashboard's "Today's leaders" list.
          // Web/desktop-mobile keep the original 6-tab bar (Movers stays
          // its own route there, unchanged).
            ...
            <>
              <nav
                ...
                style={isNative ? {
                  ...
                  borderTop: `0.5px solid ${NATIVE_COLOR.divider}`,
                } : undefined}
              >
                {primaryItems.map((n) => {
